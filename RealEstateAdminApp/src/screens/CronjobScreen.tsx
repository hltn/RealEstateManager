...
import { getCronConfig, saveCronConfig } from '../api/news-manager.api';
import { getApiErrorMessage } from '../utils/fetchPaginated';

/**
 * Screen cấu hình cronjob tự động (cào tin + lọc AI).
 *
 * Data fetching dùng React Query (KHÔNG dùng useEffect gọi API theo chuẩn project):
 * - `useQuery(['cronjob','config'])` load config hiện tại.
 * - `useMutation` lưu config → invalidate `['cronjob']` + toast.
 * Form state (isActive, frequency) là local, được sync từ query data qua effect
 * (đây là state-sync, không phải gọi API — giống pattern AuthInitializer).
 */
export default function CronjobScreen() {
  ...
  const { data } = useQuery({
    queryKey: ['cronjob', 'config'],
    queryFn: ({ signal }) => getCronConfig(signal),
    staleTime: 60_000,
    refetchOnWindowFocus: false,
  });

  // Sync config từ server vào form local (chỉ chạy khi data thay đổi).
  useEffect(() => {
    if (data) {
  ...
  const saveMutation = useMutation({
    mutationFn: () => saveCronConfig({ isActive, frequency }),
    onSuccess: () => {
      toast.success('Đã lưu cấu hình thành công!');
      void queryClient.invalidateQueries({ queryKey: ['cronjob'] });
    },
    onError: (err) => {
