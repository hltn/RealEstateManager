...
| Quy chuẩn                          |    Hiện trạng    | Ghi chú & Trạng thái                                                                                                            | Giải pháp kỹ thuật từ Neptune                                                                     |
| :--------------------------------- | :--------------: | :------------------------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------ |
| **API Versioning** (`/api/v1/...`) |   ✅ **ĐÃ CÓ**    | `main.ts` → `app.setGlobalPrefix('api/v1')`.                                                                                    | Đã verify trực tiếp, chuẩn RESTful.                                                               |
| **Swagger Documentation**          |   ✅ **ĐÃ CÓ**    | Toàn bộ Controller có `@ApiTags`, `@ApiOperation`; DTO có `class-validator`.                                                    | Rất tuyệt vời! Đầy đủ document cho FE/Mobile integration.                                         |
| **Global Error Format**            |   ✅ **ĐÃ CÓ**    | `GlobalExceptionFilter` xử lý tập trung trả về `{ statusCode, message, timestamp, path }`.                                      | Chuẩn hóa response lỗi toàn hệ thống.                                                             |
| **Response List Pagination**       |  ⚠️ **GAP GẤP**  | `getRawArticles()` và `getArticles()` trả về `{ message, data }` full list, thiếu `meta` phân trang (`total`, `page`, `limit`). | **CẦN SỬA NGAY:** Viết `PageOptionsDto` và `PaginationMetaDto` để chặn nguy cơ Tràn bộ nhớ (OOM)! |
| **RESTful URL Naming**             | ⚠️ **Chấp nhận** | Resource là danh từ số nhiều (`/raw-articles`). Các RPC-style endpoint (`/crawl`, `/publish`) dùng động từ.                     | Đúng chuẩn pattern *Action-on-Resource*. Rất hợp lý, giữ nguyên không cần đổi.                    |
...
|   #    | Gap / Khuyết điểm                   | Rủi ro hệ thống                                             | Vị trí file                     | Giải pháp khắc phục từ Neptune                                                            |
| :----: | :---------------------------------- | :---------------------------------------------------------- | :------------------------------ | :---------------------------------------------------------------------------------------- |
| **1**  | `WordPressService` là mock          | Tính năng publish chưa hoạt động thật                       | `wordpress.service.ts`          | Tích hợp `@nestjs/axios` gọi trực tiếp WP REST API kèm JWT / Application Password.        |
| **2**  | Thiếu Transaction khi Bulk Move     | Dữ liệu bị kẹt/trùng giữa 2 collection nếu server crash     | `moveRawArticlesBulk()`         | Dùng **Mongoose ClientSession (`startTransaction`)** để rollback dữ liệu an toàn.         |
| **3**  | Thiếu Idempotency-Key ở Publish     | Bị đăng trùng 2 bài lên WordPress khi mạng lag/double-click | `publishArticle`, `publishBulk` | Dùng Redis lưu `Idempotency-Key` (hoặc `urlHash`) làm khóa chặn trùng request.            |
| **4**  | Hard Delete (Xóa cứng)              | Mất sạch dữ liệu vĩnh viễn khi Admin trót lỡ tay xóa        | Cả 3 Collections                | Thêm trường `deletedAt: Date` (Soft Delete) vào tất cả Schema.                            |
| **5**  | Thiếu Audit Trail                   | Không tra cứu được lịch sử ai/khi nào đã tác động dữ liệu   | Toàn bộ Module                  | Viết Mongoose Middleware / Interceptor ghi log `createdBy`, `updatedBy`, `action`.        |
| **6**  | Thiếu Correlation ID                | Lần vết log (Tracing) cực khó khi chạy qua nhiều Service    | Toàn bộ Module                  | Dùng `AsyncLocalStorage` tích hợp `x-correlation-id` vào Logger.                          |
| **7**  | Cronjob thiếu Distributed Lock      | Chạy trùng job (Double-run) khi scale nhiều Instance        | `cronjob.service.ts`            | Tích hợp **Redlock** (Redis) hoặc **ShedLock** để khóa job giữa các instance.             |
| **8**  | Controller bị lặp `try/catch`       | Vi phạm nguyên lý DRY, code rác nhiều                       | Controller                      | Bỏ `try/catch`, đẩy hết lỗi cho `GlobalExceptionFilter` xử lý tự động.                    |
| **9**  | Response List thiếu Meta/Phân trang | API bị treo, lag, giật khi database phình to                | `getRawArticles`, `getArticles` | Áp dụng `PageOptionsDto` + `.skip()`, `.limit()` (như Neptune đã hướng dẫn ở bước trước). |
| **10** | Thiếu Retry / Exponential Backoff   | Job thất bại vô lý khi API bên thứ 3 bị chập chờn 1-2s      | Crawler & AI Service            | Dùng thư viện **P-Retry** hoặc **Axios-Retry** với cơ chế Exponential Backoff.            |